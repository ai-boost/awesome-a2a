...
*   🌟 [Chitin](https://chitin.id) by [@chitin-id](https://github.com/chitin-id) [![Stars](https://img.shields.io/github/stars/chitin-id/chitin-contracts?style=social)](https://github.com/chitin-id/chitin-contracts) - On-chain identity layer for AI agents with A2A readiness detection. Provides ERC-8004 passports + soulbound tokens on Base L2, DID resolution (`did:chitin:8453:{name}`), and an [A2A readiness API](https://chitin.id/api/v1/agents/{name}/a2a-ready) that resolves agent endpoints from ERC-8004 service registrations. Also available as an [MCP server](https://www.npmjs.com/package/chitin-mcp-server).
