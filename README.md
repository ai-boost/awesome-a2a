...
*   📊 [Double Agent](https://agentagent.resolved.sh) by [@hichana](https://github.com/hichana) [![Stars](https://img.shields.io/github/stars/hichana/double-agent?style=social)](https://github.com/hichana/double-agent) - Competitive intelligence agent for the x402 ecosystem: tracks 300+ agent-economy companies via the coinbase/x402 GitHub PR stream and sells structured JSONL datasets (full index, weekly diffs, vetted-only, raw) via autonomous x402 USDC payment on Base. Live A2A agent card at [agentagent.resolved.sh/.well-known/agent-card.json](https://agentagent.resolved.sh/.well-known/agent-card.json).
