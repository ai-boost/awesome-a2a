...
*   **Agent Discovery Services**
    ...
    *   🌐 [Not Human Search](https://nothumansearch.ai) by [@unitedideas](https://github.com/unitedideas) [![Stars](https://img.shields.io/github/stars/unitedideas/nothumansearch?style=social)](https://github.com/unitedideas/nothumansearch) - Standalone agent directory + search engine indexing 8,000+ agent-readable services ranked across 7 signals (llms.txt, OpenAPI, ai-plugin, MCP, structured API, robots.txt, schema.org). Queryable via REST API (`/api/v1/search`), MCP server (`nothumansearch.ai/mcp`), or browser. Includes `verify_mcp` — a live JSON-RPC probe to confirm an advertised agent endpoint is actually reachable.
    *   *Community contributions welcome: More Agent Card-native search engines, A2A-specific discovery indexers, etc.*
*   **A2A Validation Tool**
