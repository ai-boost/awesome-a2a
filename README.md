...
*   **Agent Discovery Services**
    ...
    *   ⚙️ [Alya Hub](https://mydaughteralya.com) by [@Rupert1987](https://github.com/Rupert1987) [![Stars](https://img.shields.io/github/stars/Rupert1987/alya-mcp?style=social)](https://github.com/Rupert1987/alya-mcp) - Cross-protocol agent hub bridging A2A and MCP. Exposes `/.well-known/agent.json` (A2A AgentCard) plus an MCP `agent_registry` tool for `list`/`lookup`/`call_external_agent`. Live registry with REST + SSE feed, telemetry, and per-call billing ($0.001/call, first $5 free). Listed on the official MCP Registry as `io.github.Rupert1987/alya-hub`. NPM stdio client at `@mydaughteralya/mcp`. MIT.
*   **A2A Validation Tool**
